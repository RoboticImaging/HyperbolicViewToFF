...
function FigureHandle = LFDispVidCirc_GIF( LF, outfile)

%---Defaults---
PathRadius_percent = LFDefaultVal( 'PathRadius_percent', 70 );
...
%---Setup the display---
% [ImageHandle,FigureHandle] = LFDispSetup( squeeze(LF(floor(end/2),floor(end/2),:,:,:)) );
...
while(t<20)
    ...
    if t==0
        ...
        imwrite(imind,cm,outfile,'gif','DelayTime',.05,'writemode','append');
    end
